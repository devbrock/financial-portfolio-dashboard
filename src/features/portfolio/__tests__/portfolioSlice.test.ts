...
describe('portfolioSlice', () => {
  ...
  it('updates a holding when id matches', () => {
    ...
    const next = reducer(state, updateHolding({ id: 'h1', updates: { quantity: 42 } }));
  ...
  it('ignores updates when holding id is missing', () => {
    ...
    const next = reducer(state, updateHolding({ id: 'missing', updates: { quantity: 42 } }));
    expect(next.holdings[0].quantity).toBe(10);
  });

  it('updates user preferences', () => {
    const next = reducer(baseState, updatePreferences({ theme: 'dark', currency: 'USD' }));
